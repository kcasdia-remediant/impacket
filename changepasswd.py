...
class SamrPassword(PasswordHandler):
    ...
    def hSamrOpenUser(self, username):
        ...
        try:
            ...
            domainSID = samr.hSamrLookupDomainInSamServer(self.dce, serverHandle, self.address)["DomainId"]
    ...
    def hSamrUnicodeChangePasswordUser4(self, servername, username, oldPassword, newPassword):
        return self._SamrWrapper(
            samr.hSamrUnicodeChangePasswordUser4,
            servername,
            username,
            oldPassword,
            newPassword,
    ...
    def _changePassword(
        ...
        if newPassword:
            servername = "\x00"
            if self.address:
                servername = self.address.split('.')[0].upper()
            # If using a plaintext value for the new password
            return self.hSamrUnicodeChangePasswordUser4(
                servername, targetUsername, oldPassword, newPassword
            )
        else:
